...
public class TodayInput extends JFrame {
	...
	private JButton btnPhoto, btnSave, btnBack;
...
//		setComboTransparent(comboDay);
		...
		scrollPane.setBounds(12, 155, 436, 223);
		pn1.add(scrollPane);
		...
		lblBackground.setIcon(new ImageIcon("D:\\springGreen\\java\\\\works\\springGreen5\\images/000_00.jpg"));
		//lblBackground.setIcon(new ImageIcon("images/000_00.jpg"));
...
//		comboYear.setOpaque(false);
		...
		btnBack.setBounds(330, 275, 83, 33);
		pn2.add(btnBack);
		btnBack.setOpaque(false);
		btnBack.setFont(new Font("휴먼아미체", Font.BOLD, 20));
...
//------------------------------------	
		...
		btnSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// 오늘날짜가 아니면 이미 등록된 일기장인지 확인후 등록되어 있는 일기장은 다시 재등록할 수 없도록 처리한다.
				if(!ymd.equals(wDate)) {
				...
				if(vo.getPhoto() != null) photo = vo.getPhoto();
				...
				if(!ymd.equals(wDate)) {
					...
					if(ans == 0) {
						TodayVO vo2 = dao.getTodaySearch(ymd);
						if(vo2.getwDate() == null) res = dao.setTodayInput(vo);
						else res = dao.setTodayUpdate(vo);
						
						if(res != 0) JOptionPane.showMessageDialog(null, "오늘 일기장을 작성(수정) 했습니다.");
